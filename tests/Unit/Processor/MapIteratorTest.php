...
use Acelot\AutoMapper\Processor\MapIterator;
...
/**
 * @covers \Acelot\AutoMapper\Processor\MapIterator
 */
final class MapIteratorTest extends TestCase
{
    public function testProcess_NotUserValuePassed_NeverCallsProcessorProcessMethod(): void
    {
        ...
        $processor = new MapIterator($subProcessor);

        self::assertSame($value, $processor->process($context, $value));
    }

    public function testProcess_NotIteratorPassed_ThrowsUnexpectedValueException(): void
    {
        $context = $this->createMock(ContextInterface::class);
        $subProcessor = $this->createMock(ProcessorInterface::class);

        self::expectExceptionObject(new UnexpectedValueException('Iterator', []));
    ...
    public function testProcess_KeepKeysAndSomeElementsIgnored_ReturnsIteratorWithNotIgnoredElements(): void
    {
        ...
        $processor = new MapIterator($subProcessor, true);
